...
public class listFactura2 extends JFrame {
	 ...
	 * Create the frame.
	 */
	public listFactura2(int idC,Connection con) {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(index != -1) {
					...
					if(seleccion == 0) {
						...
						modificarFactura(idF,idC,tipoP,con);			
	...
	public void mostrar(int idC,Connection con){
		...
		try {
			...
			st = con.prepareStatement("select ID_Factura,TipoDePago,Fecha_Creacion,Nombres,Apellidos,Email,Telefono from Factura join cliente on Factura.Cliente = Cliente.ID_Cliente where Factura.Cliente = ? order by Fecha_Creacion");
	...
	private void modificarFactura(int idF,int idC,String tipoP,Connection con) {
