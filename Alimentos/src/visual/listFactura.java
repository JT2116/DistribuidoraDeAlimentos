...
public class listFactura extends JFrame {
	...
	public void mostrar(Connection con){
		...
		try {
			...
			datos = st.executeQuery("select ID_Factura,TipoDePago,Fecha_Creacion,Nombres,Apellidos,Email,Telefono from Factura join cliente on Factura.Cliente = Cliente.ID_Cliente order by ID_Factura");
			
			while(datos.next()) {				
				tableFactura.addRow(new Object[]{datos.getInt("ID_Factura"),datos.getString("TipoDePago"),datos.getString("Fecha_Creacion"),datos.getString("Nombres")+" "+datos.getString("Apellidos"),datos.getString("Email"),datos.getString("Telefono")});
			}
